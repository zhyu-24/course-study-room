...
复杂输入经过一个系统以后，会产生什么输出？本讲前半部分学习表示复杂信号的基本单元，后半部分研究允许响应叠加与时移的系统。把两部分联系起来，就能理解单位冲激为什么能成为系统分析的基础。复习时先沿主线阅读，再用每节自测检查概念、公式与波形之间的对应。
...
反折与平移的顺序改变后，参数也需要相应调整。 对已经形成的 \(g(t)=f(t-1)\) 反折，得到 \(g(-t)=f(-t-1)\)。若先构造 \(h(t)=f(-t)\)，再把它延时 1，则 \(h(t-1)=f(-t+1)\)，两者不同。可以选择不同操作路径，但需要相应调整平移方向和距离。
...
这组欧拉关系把正弦振荡与复指数联系起来，之后进行变换域分析时会反复使用。抽样函数的命名与作用会在后续学习中进一步联系起来。

> 本节的两条主线：把复杂信号分成简单信号；研究允许组合和时移响应的 LTI 系统。前者给“积木”，后者给“怎样组装”的规则。

[课堂回顾 T01–T02](Transcript_Corrected.md#T01) · [来源](Lecture_Source_Map.md)
...
奇异信号的函数本身或相关导数、积分具有不连续等特殊性质。这些是从实际过程抽象出来的理想模型。例如斜变函数本身在零点连续，但斜率在那里改变。
...
它保留自变量的正负信息，是奇函数。它也可以用来表示绝对值： \(|t|=t\operatorname{sgn}(t)\)，在零点也可按 \(\operatorname{sgn}(0)=0\) 延拓。

[课堂回顾 T03–T06](Transcript_Corrected.md#T03) · [来源](Lecture_Source_Map.md)
...
[课堂回顾 T07–T09](Transcript_Corrected.md#T07) · [来源](Lecture_Source_Map.md)
...
\(e^{-t}\delta(t-2)=e^{-2}\delta(t-2)\)；其全轴积分为 \(e^{-2}\)。不要在乘法这一步就把 \(\delta\) 丢掉。
...
可以用变量替换推导这个系数。 对测试函数 \(\phi\)，令 \(s=at\)。当 \(a>0\)，积分上下限不换序：
...
注意：单独把自变量换为 \(at\)，积分强度会变成 \(1/|a|\)。若希望压缩之后面积仍为 1，还要乘 \(|a|\)。这两种操作对积分强度的影响不同。
...
对矩形脉冲 \(u(t-T)-u(t-2T)\) 求导，得到 \(\delta(t-T)-\delta(t-2T)\)。开启处是正跳变，关闭处是负跳变，冲激强度分别等于跳变量。

[课堂回顾 T10–T11](Transcript_Corrected.md#T10) · [来源](Lecture_Source_Map.md)
...
这里要求 \(f\) 在冲激位置有足够的光滑性。\(\delta'\) 对常数测试函数的作用为零，因此全轴积分意义下 \(\int\delta'(t)dt=0\)。对上式积分便得到
...
令 \(g(t)=f(t-t_0)\)，则 \(g'(0)=f'(-t_0)\)，第二式就来自 \(-g'(0)\)。两个式子的取样点不同：前者取 \(t_0\)，后者取 \(-t_0\)。相应的光滑条件都应满足在实际取样点附近；课件第二式旁标出的条件位置需要据此调整。
...
[课堂回顾 T12–T14](Transcript_Corrected.md#T12) · [来源](Lecture_Source_Map.md)
...
把平方展开，就能看出交叉项怎样消失。 对实信号，按单位电阻归一化平均功率，
...
课件用“虚部分量”指 \(jf_i\)。计算时须区分虚部数值 \(f_i\) 与带有虚数单位的分量 \(jf_i\)。复信号是计算工具，可同时表达振幅与相位等信息，不必把它理解成直接测到一个“虚电压”。
...
把 \(t\) 换成 \(-t\)，可直接验证 \(f_e(-t)=f_e(t)\)、\(f_o(-t)=-f_o(t)\)。相加时反折项抵消，恢复原函数。读图时分别检查偶分量左右镜像是否相同、奇分量反折后是否变号，最后叠加是否恢复原图。

[课堂回顾 T15–T16](Transcript_Corrected.md#T15) · [来源](Lecture_Source_Map.md)
...
这里 \(t\) 是输出表达式留下来的变量，\(\tau\) 是积分掉的哑变量。对固定 \(t\)，冲激抽取 \(\tau=t\) 的值，因此积分又等于 \(x(t)\)。对通常连续的信号，这给出了冲激分解的表示；遇到间断点，还需结合取值约定或广义意义理解。
...
这也为卷积建立了基础。 设单位冲激响应为 \(h(t)\)，则时移冲激对应 \(h(t-\tau)\)。在交换积分与系统作用合法的条件下，组合得到 \(y(t)=\int_{-\infty}^{\infty}x(\tau)h(t-\tau)d\tau\)。本堂课先理解这条思路，具体卷积方法留到后续。

除了脉冲分解，还可以选择其他基本单元。后续的正交分解和傅里叶分析会继续发展这一思路。

[课堂回顾 T17–T18](Transcript_Corrected.md#T17) · [来源](Lecture_Source_Map.md)
...
沿信号流向读图： 输入先乘 \(1/L\)，反馈电流乘 \(R/L\)，在求和点相减，所得是 \(di/dt\)；通过积分器得到电流 \(i(t)\)。积分器的初值决定初始电流。另一种等价画法是先计算 \(x-Ri\)，再经过 \(1/L\) 和积分器。

> 读这张图时需核对增益所在的支路：先对输入和反馈分别乘增益，与先求差再乘增益，是两种不同画法。应确保最后得到上面的RL方程。
...
教师明确区分两种要求：一般不会要求从复杂物理电路直接建模，但**给定框图，要会写输入输出关系和相应微分/差分方程**。练习时要落实到每条支路的运算与求和点的正负号。

[课堂回顾 T19](Transcript_Corrected.md#T19) · [来源](Lecture_Source_Map.md)
...
判断全系统因果性不能只检查某个正时刻；要求对所有允许时刻成立。例如反折系统在负时间需要未来输入，已足以判定它在全时间轴上非因果。

对称滑动平均会用到当前点两侧的数据，因此可能非因果；过去数据的单边滑动平均可以因果。不能把所有滑动平均一概归为非因果。预先录好的数据可离线处理，所以非因果并不等于“毫无用途”。

### 9.2 可逆性：输出是否丢失了信息

\(y=2x\) 可由 \(x=y/2\) 恢复，是可逆的。\(y=x^2\) 对不限制正负的实输入不可逆，因为 \(x\) 与 \(-x\) 输出相同。若题目限制输入非负，平方在这个受限集合上可逆；因此可逆性也与允许的输入集合有关。
...
[课堂回顾 T20–T21](Transcript_Corrected.md#T20) · [来源](Lecture_Source_Map.md)
...
以 \(2x+3\) 为例，可以把输入相关项与固定项分别处理，但固定映射 \(H[x]=3\) 本身不满足线性定义。把响应拆开与判定各部分是否线性，是两个问题。后续双零法会进一步说明初始状态响应和零状态响应如何分别求解。

[课堂回顾 T22](Transcript_Corrected.md#T22) · [来源](Lecture_Source_Map.md)
...
课件例子标有 \(t>0\)，这里的判别取输入与时移均有定义的共同范围；这一范围限制本身不表示系统另有固定开机时刻。若题目另加固定开机时刻或非零初始条件，应重新检查其影响。
...
[课堂回顾 T23](Transcript_Corrected.md#T23) · [来源](Lecture_Source_Map.md)
...
<details markdown="1"><summary>反例：固定延迟与积分下限</summary>
...
本讲建立的对象和基本性质，是后续学习这些方法的基础。它们共通的思路仍是：选择适当基本信号，先研究基本响应，再重组。

[课堂回顾 T24–T25](Transcript_Corrected.md#T24) · [来源](Lecture_Source_Map.md)
...
当前归档的**第三版上册**中，1-14 指定小题和 1-18(c) 图形在 PDF p.59 / printed p.43；1-20、1-23、1-24 在 PDF p.60 / printed p.44。使用第四版时，需要核对题面，少数习题可能与第三版不同。
...
- 使用公式时，先检查积分下限、初始状态和信号的定义范围。

[课堂回顾 T25](Transcript_Corrected.md#T25) · [重要订正](Lecture_Source_Map.md#corrections) · [来源](Lecture_Source_Map.md)
