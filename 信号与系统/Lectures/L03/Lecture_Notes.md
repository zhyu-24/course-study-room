...
> 本讲按同一天的上下两段素材合并归档。上段止于 RC 响应分解，下段从因果卷积积分接续；课件 p.19–25 的中间推导没有对应转写，已明确标为课件补充。课堂可确认讲至 p.53；p.54–56 为未讲预览，不能把整份课件当作全部已授内容。日期来自转写头部。资料状态 draft；原音尚未核听。

本讲要解决的问题是：**给定系统、输入和起始状态，怎样求输出？** 前半段用微分方程直接求解，后半段利用线性与时不变性，把任意输入分解为冲激，再由冲激响应组合出输出。复习时应能亲手完成二阶方程例题、分清两种响应分解，并用图解与性质两种方法计算卷积。

## 阅读与原始材料

- [课堂回顾](Transcript_Corrected.md) 保留授课顺序、例子、问答和学习要求；时间均为各段原转写定位。
- [来源与订正](Lecture_Source_Map.md) 包含两段对齐、全部课件页覆盖、重要差异和教材定位。
- [原课件 Ch2.pdf](Raw/Ch2.pdf)；教材为郑君里等《信号与系统》第三版上册，课件推荐第四版，题号需按实际版本核页。

正文中的“课件补充”“补充推导”均为编者整理，不能视作缺失录音中的教师原话。其余主线由课件事实与可读转写支持。
...
[对应课堂 T01–T03](Transcript_Corrected.md#T01) · 来源：课件 p.1–4、上段 00:03–06:45。
...
[对应课堂 T04–T05](Transcript_Corrected.md#T04) · 来源：课件 p.5–6、上段 06:45–11:56。
...
**补充推导：** 共轭根 \(\alpha\pm j\beta\) 的实解为 \(e^{\alpha t}(B\cos\beta t+C\sin\beta t)\)。若根 \(a\) 重复 \(q\) 次，该部分要写成 \((A_0+A_1t+\cdots+A_{q-1}t^{q-1})e^{at}\)，不能仍只写一个指数。这补足课件“无重根”条件。
...
这些是待定系数法的常用候选。**特解系数靠方程两边恒等来确定，不靠初始条件。** 若候选与齐次解重合，应乘适当的 \(t\) 次幂后再试；若模型右端含输入导数，应先写出实际右端再选形式。这两点为编者补充适用条件。
...
[对应课堂 T06–T09](Transcript_Corrected.md#T06) · 来源：课件 p.7–13；教材 PDF p.76 对自由响应系数的说明。
...
**补充推导：** 对 \(r'+ar=b\delta(t)\)，假设 \(r\) 只含有限跳变而无冲激本身，对零点两侧积分：
...
转写用“零时刻无穷大”直观说明冲激；严格地说，\(\delta\) 是由积分性质定义的广义函数，不是普通的有限跳变函数。后续单边拉普拉斯法可以直接引入 \(0^-\) 数据，**并非忽略跳变，而是把其作用纳入计算。**

[对应课堂 T09–T10、T13](Transcript_Corrected.md#T09) · 来源：课件 p.9–10、14。
...
[对应课堂 T11–T12](Transcript_Corrected.md#T11) · 来源：课件 p.12、上段 21:12–27:17。
...
> 订正：上段 32:07 转写为特征根“−1”。此处按课件的元件值及方程采用 −3；无法在未核听情况下判断是 ASR 错误还是口头差异。

**以下零输入/零状态细算由课件 p.19–20 补足，恰落在素材断点，不冒充原话。** 零输入时 \(r_{zi}'+3r_{zi}=0\)，保留 \(3/2\) 初值；零状态时 \(r_{zs}'+3r_{zs}=3\)，初值为零。因此在 \(t\ge0\)：
...
[对应课堂 T14–T17](Transcript_Corrected.md#T14) · 来源：课件 p.15–20、上段 27:59–录音末端；教材 PDF p.76 的定义。
...
<a id="N07"></a>
## 7. 课件补充：零状态线性与“固定初始状态”

> 对应 p.21–22。现有转写没有覆盖，以下只作为课件支持的复习内容。
...
[断点说明](Transcript_Corrected.md#gap) · 来源：课件 p.21–22。
...
> 定义及冲激分解的详细链条来自断点处课件 p.23–26；下段 00:00 起的讲授确认因果积分限并回顾此结论。
...
[对应课堂 T18–T20](Transcript_Corrected.md#T18) · 来源：课件 p.23–28、34；教材 PDF p.80。
...
> 课件 p.32 的求和从 \(k=1\) 起，会漏掉 \(\delta\) 本身，与同页 \(m=n\) 的说明不一致。这里改为从 0 起，并记录在订正表。此处讲的是本课常系数因果模型，不是说任意 LTI 响应天然都乘 \(u(t)\)。

[对应课堂 T20–T23](Transcript_Corrected.md#T20) · 来源：课件 p.28–32；教材 PDF p.80。
...
[对应课堂 T24–T28、T32](Transcript_Corrected.md#T24) · 来源：课件 p.33–36、41。
...
读图：输入在 2 时归零，输出仍有指数尾部，这是储能延续的表现。**补充复算：** 前一段导数在 \(t=2\ln2\) 为零，真实峰值在 \(2\ln2\approx1.386\)，不是在截断时刻 2；课件曲线是示意图，应以公式为准。两段公式在 \(t=2\) 连续。

教师明确要求课后把该例独立算一遍；这里保留课堂例题解法，后面的指定教材作业另列。

[对应课堂 T29–T31](Transcript_Corrected.md#T29) · 来源：课件 p.37–40、下段 18:42–22:55。
...
**补充推导，展开课件给出的结果：** 取 \(f_1(\tau)\) 为固定函数，则
...
首段的积分为 \([t\tau/2-\tau^2/4]_{-1}^{t}\)，其余同理。复算结果在 −1、1、2、4 连续，在 2 达到最大值 2。教师未在转写中逐行展开计算，而是指出后面可利用卷积性质简化。

[对应课堂 T32–T33](Transcript_Corrected.md#T32) · 来源：课件 p.41–42、下段 22:55–25:20。
...
> 下段 27:10 的转写把分配律第二项记为 \(f_2*f_3\)，按课件 p.43 订正为 \(f_1*f_3\)。

[对应课堂 T34–T36](Transcript_Corrected.md#T34) · 来源：课件 p.43–47。
...
课堂问答：\(\delta\) 是偶分布，\(\delta'\) 是奇分布。平移冲激产生时间平移，不是转写中“频率影响”。导数含跳变时应保留冲激项。
...
[对应课堂 T37–T41](Transcript_Corrected.md#T37) · 来源：课件 p.48–52；教材 PDF p.92。
...
**补充推导，解释边界项：** 对适当可积、分段光滑的函数，若 \(f(-\infty)\) 存在，
...
课堂最后以“负无穷处不为零”解释，并保留了“我不保证全对，大概是这样”的口头限定。教材第三版 PDF p.92、式(2-70)后也强调时间受限及负无穷的条件。这里给出的是编者展开的数学说明，不把课堂探索性说法写成无条件定理。满足零边界还须确认卷积、积分和换序本身合法。

[对应课堂 T41–T43](Transcript_Corrected.md#T41) · 来源：课件 p.53；教材 PDF p.92 / 印刷 p.76。
...
以上保留任务与定位，不默认代做全部作业。第四版尚未提供，不能宣称两版全部题面一致。尤其 2-19(b) 的第一个信号在负时间也为 1，不可漏成普通因果阶跃；它与上一节边界项问题正好有关。

![课堂小结、作业、题面更正与预习要求](assets/slides/p-58.jpg)

### 未讲内容与预习

下段 41:54 明确说用冲激响应分析因果性留到下次，让大家先自学。因此课件 p.54–56 的因果性、稳定性判据只保留为未讲预览，不编入本讲教师整理稿。p.57 是课件要求汇总，无法确认课堂逐项讲过。p.59 为致谢。

课件预习范围：**§3.1–§3.3**。本讲不是第二章全部教材内容的逐节授课记录；教材的多径消除与算子法等不因章名相同就自动补成已讲内容。

[对应课堂 T44](Transcript_Corrected.md#T44) · [教材导航](../../Global/Textbook_Index.md) · [完整来源与待核听项](Lecture_Source_Map.md)

<details markdown="1"><summary>复习闭环：能回答这四个问题再去做作业</summary>
