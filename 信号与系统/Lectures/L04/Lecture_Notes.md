...
为什么把信号拆成不同频率的正弦波，会更容易分析？本讲先完成LTI系统因果性与稳定性的讨论，再由复指数响应引出傅里叶级数。沿着系数计算、频谱、对称性和有限项逼近这条线，理解周期、脉宽与频谱结构之间的关系。
...
冲激分解采用全实轴积分。若把冲激放在积分端点，需另行约定端点取值；这与因果卷积可将积分上限取为当前时刻的理由不同。
...
先完成上讲留下的问题：能否仅从冲激响应判断系统的因果性与稳定性？可结合[上讲课件第54–56页](../L03/Raw/Ch2.pdf#page=54)阅读。
...
当 \(\tau>t\) 时，\(t-\tau<0\)，核为零；所以输出只用现在和过去的输入。反过来，因果系统在输入冲激到来之前不能产生其零状态响应，因此 \(h(t)\) 在负时间必须为零。这里“因果信号”是描述负时间为零的波形，不是说任意波形自身构成系统。
...
注意不能漏掉输入界 \(M\)，且必须是**绝对可积**；正负抵消后的积分有限不足以控制任意输入。充要判据涉及所有有界输入，不能只试一个阶跃就断定稳定。含冲激直通项的系统要用相应广义解释，不能把 \(\delta\) 当普通函数取绝对值积分。

用三个冲激响应比较：\(h=e^{-t}u(t)\) 因果且绝对积分为1；\(h=u(t)\) 因果但不稳定，单位阶跃输入得到 \(t u(t)\)；\(h=e^t u(-t)\) 非因果但绝对积分为1，稳定。因果性与稳定性是两件事。
...
对实系统的全时间正弦输入，或适用的正弦稳态分析，同样可读成“同频率，改变幅度与相位”。具体应用时还要注意：若输入在0时刻才接入 \(\cos\omega t\,u(t)\)，输出通常还有启动暂态，不能一概只写同频余弦。虚指数模恒为1也不等于任意系统都稳定。
...
教师强调本章是核心，特别要求长期记住卷积定理与抽样定理；后续将逐步展开这两个定理。学习目标不是重复微积分课程的全部收敛性证明，而是把数学工具用于分析信号和系统。

课件p.8–10以傅里叶的热传导研究、变形金刚和蝴蝶图示说明“换一种表示以取得好处”。教师又用 Radon 变换与 CT 成像说明理论可能早于应用，用 FFT 将直接计算的 \(O(N^2)\) 降到典型 \(O(N\log N)\) 的例子说明计算工具的重要性。

傅里叶分析关心如何在合适函数空间和条件下进行分析与重建。可逆性使两种表示能够相互恢复，但一般数学中的变换并不都要求可逆，非单射映射也可称为变换。
...
要求非零谐波系数，可以把展开式两边乘 \(\cos(m\omega_1t)\) 再积分。不同次数的余弦互相正交，正弦与余弦也正交；只有 \(n=m\) 的余弦项留下，且
...
在连续点，级数收敛到 \(f(t)\)；在有限跳变点，标准傅里叶部分和的极限为
...
第一图每个台阶的宽度和高度依次减半，展示无限多个间断点；第二图 \(\sin(2\pi/t)\) 在0附近振荡越来越密，展示无限多个极值；第三图 \(1/t\) 在 \(0<t\le1\) 的周期延拓不满足绝对可积条件。不满足充分条件后，仍须具体判断。第三例连直流积分 \(\int_0^1dt/t\) 都发散，不具有本讲定义的普通傅里叶级数系数。
...
依据是展开右边得到 \(a_n=c_n\cos\varphi_n\)、\(b_n=-c_n\sin\varphi_n\)。普通 \(-\arctan(b_n/a_n)\) 需要补象限、处理分母为0；\(c_n=0\) 的谱线相位无定义。另一种紧缩式为 \(d_n\sin(x+\theta_n)\)，其中 \(d_n=c_n\)、\(\theta_n=\operatorname{atan2}(a_n,b_n)\)。
...
课件p.18–19的矩形脉冲从0延续到标记0.3的位置，并非以原点为中心的偶脉冲，因此一般同时有 \(a_n,b_n\) 和非平凡相位。幅值1、每周期在 \(0<t<d\) 非零时，
...
对有限和平方后积分，交叉项因正交性消失；直流项平方平均仍为 \(a_0^2\)，每个余弦/正弦平方平均为其幅度平方的一半。对适用的平方可积信号再取均方极限。指数形式中 \(e^{jn\omega_1t}\) 的模平方为1，于是每条谱线贡献 \(|F_n|^2\)。
...
对于半周期重复的信号，若 \(T_1\) 原本声称为最小周期，半周期重复意味着需要重新确定基波。

偶信号乘正弦为奇函数，对称区间积分为0；奇信号乘余弦同理。半波反对称时将系数积分拆成两半并平移，第二半是第一半乘 \(-e^{-jn\pi}=-(-1)^n\)，总因子为 \(1-(-1)^n\)，偶数次全部消失。这不要求信号关于原点为奇函数。

![半波反对称的读图规则](assets/slides/p-34.jpg)

图上把波形沿时间轴平移半周期，再上下反转，正好与原波形重合。判定前要看清选定的时间原点；同一个形状平移后可能失去奇偶对称，但半波关系仍可保留。

幅度约定为非负实数时，偶信号取 \(c_n=|a_n|\)，奇信号取 \(c_n=|b_n|\)。奇实信号的指数系数 \(F_n\) 为纯虚数，幅度 \(c_n\) 仍为实数。
...
图中左侧脉冲逐渐稀疏，右侧谱线却逐渐密集。注意这不是“信号越稀疏，谱线也越稀疏”。固定脉宽时，第一包络零点始终为 \(2\pi/\tau\)。
...
与下一讲联系起来看：固定单脉冲形状令 \(T_1\to\infty\)，谱线变密、\(F_n\) 变小，但 \(T_1F_n\) 对应有限的 \(G(\omega)\)，由此引出连续频谱。不能直接把未缩放的“级数包络”说成同幅度的傅里叶变换。若固定 \(T_1\) 令 \(\tau=T_1\)，信号为常数 \(E\)，则 \(F_0=E\)、\(F_{n\ne0}=0\)，不是无限大的级数系数；常数的广义傅里叶变换为冲激是另一种表示。
...
读此页时应注意平台幅值：图示为 \(\pm E\) 时，级数前因子应为 \(4E/\pi\)；页中所列 \(2E/\pi\) 对应 \(\pm E/2\) 的平台。

统一记平台幅值为 \(\pm A\)，在 \(|t|<T_1/4\) 为 \(+A\)，另半周期为 \(-A\)。把它写作高度 \(2A\)、占空比1/2的单极性脉冲减去常数 \(A\)，于是
...
这里的“最小”有明确的比较范围：在固定的这些基函数张成的空间内，傅里叶系数使误差与所有保留基函数正交；任取另一组系数，相当于在正交残差外再加一个非零误差分量，平方范数只会增加。它是均方意义最优，不保证每一点误差同时最小。
...
把本讲放在傅里叶分析的整体框架中，可以看到四类表示的联系。离散时间部分还会继续学习后两项：
...
课件p.47指定 **3-4、3-5、3-6、3-9**；p.45提示课下推导周期锯齿、三角、半波余弦、全波余弦，注意对称性。当前归档是第三版上册，课件推荐第四版，以下为第三版实际题面，使用第四版时需核对题面。
