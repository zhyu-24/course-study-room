...
这一讲先用每单位时间的衰变概率描述核素寿命，再用每单位路程的碰撞概率描述中子运动。两条线都导向指数规律。通过活度、断代、核密度与截面的算例，逐步分清衰变常数、宏观截面、平均寿命和平均自由程。
...
铀-235本身是长寿命放射性核素，不是严格稳定核。更完整的机制是
...
[T01–T02](Transcript_Corrected.md#T01) · [来源](Lecture_Source_Map.md)
...
课堂问：同一核素从激发态退激到基态，为什么不放出一个质子或中子？答：若质子数、中子数都没变，就不是通过发射核子来完成的那条跃迁；课件讨论的是γ退激。γ也是光子。还要注意，核退激还有内转换等方式，“只能发γ”并非一般定理；γ与X射线也不能只按能量高低划界。本讲不展开这些支路。
...
设\(N(t)\)是时刻\(t\)尚未衰变的某种母核数目，\(\lambda_d\)为衰变常数，单位\(\mathrm{s^{-1}}\)。下标d用于区分衰变常数与自由程；课件写作\(\lambda\)。在衰变常数不变、没有外来补充或其他移除的模型下，
...
****平均寿命需用衰变时刻的概率密度\(f(t)=\lambda_de^{-\lambda_dt}\)加权，不是直接以\(N(t)\)当成衰变数：

\[\bar t=\int_0^\infty t\lambda_de^{-\lambda_dt}\,dt
=\left[-te^{-\lambda_dt}\right]_0^\infty+\int_0^\infty e^{-\lambda_dt}\,dt
=\frac1{\lambda_d}.\]

因此\(T_{1/2}=0.693\bar t\)，平均寿命更大。例如\(\lambda_d=0.02\,\mathrm{s^{-1}}\)，平均寿命50 s，半衰期约34.7 s。
...
课堂以铯-137的β衰变及随后子核的γ退激说明：一次母核衰变可伴随后续射线发射，不能把β数、γ数简单相加作为铯-137的活度。若讨论包含子体的总样品活度，应另行声明计数口径。图中的分支比例用于说明衰变过程，精确计算应采用相应核数据。
...
[T06–T07](Transcript_Corrected.md#T06) · [来源](Lecture_Source_Map.md)

<a id="N05"></a>

## 5. 人体钾-40活度算例与丰度的口径

题设：体重75 kg，钾质量占0.2%，天然钾中钾-40原子丰度为0.0117%，半衰期\(1.277\times10^9\)年。计算采用题设的理想模型。
...
课件用40代替平均摩尔质量是数量级估算。因此约4500 Bq是这一近似下的估算结果。

课堂另给：人体含18%碳，碳-14丰度\(1.2\times10^{-12}\)，半衰期5730年，要求课后加入碳-14计算总活度；课件给出的参考总量约7900 Bq。可按上面的方法继续计算。关键思考是：核数虽少，衰变常数却大得多，也能贡献显著活度。
...
[T08–T10](Transcript_Corrected.md#T08) · [来源](Lecture_Source_Map.md)
...
课堂讨论恐龙化石：经过极多半衰期，原始碳-14已少到不适合该方法，还必须判断样品是否保存待测的原始碳。不能只因为“物体古老”就套公式。
...
[T11–T12](Transcript_Corrected.md#T11) · [来源](Lecture_Source_Map.md)

<a id="N07"></a>

## 7. 射线、丰度与铀钍的长期衰变

课件42页比较射线的物理组成：α为氦原子核，β为电子或正电子，γ为光子，中子射线为运动的中子，质子为氢原子核。带电粒子能直接电离；光子和中子主要通过产生次级带电粒子间接电离。贯穿能力依粒子能量和介质而变，课件给出的是定性比较，不能由“电离本领弱”推断生物效应小，也不能由皮肤能阻挡某些α粒子推出一般照射无害。
...
[T13–T15](Transcript_Corrected.md#T13) · [来源](Lecture_Source_Map.md)
...
[T16–T18](Transcript_Corrected.md#T16) · [来源](Lecture_Source_Map.md)
...
[T19–T21](Transcript_Corrected.md#T19) · [来源](Lecture_Source_Map.md)
...
在各出口以常数衰变率竞争的模型中，\(\lambda_d=\sum_i\lambda_i\)，乘以\(\hbar\)就得到总宽度为分宽度之和；相应分支比\(b_i=\Gamma_i/\Gamma\)。不能仅凭“跃迁能量更大”断定某支路的概率一定更大。
...
[T24–T27](Transcript_Corrected.md#T24) · [来源](Lecture_Source_Map.md)
...
常用下标：总截面t、散射s、吸收a、辐射俘获γ或c、裂变f。在当前散射/吸收的简化分类中\(\sigma_t=\sigma_s+\sigma_a\)，吸收包括俘获、裂变及适用的其他吸收道。**分类边界：**PPT68把\((n,2n)\)也列入“吸收”式，属于需澄清的反应道记账口径；它不能直接当作中子净损失，后续输运中必须计入其倍增源，不能机械套用两项损失分解。

![氢、氧截面随能量变化](assets/slides/p-69.jpg)

图中红线为氢的吸收截面、蓝线为氧-16总截面，比较的是不同反应道。低能区红线随能量升高而下降，蓝线高能区出现结构，说明截面具有能量依赖。读图时先辨明能量范围与反应道，再比较截面的变化。
...
[T28–T30](Transcript_Corrected.md#T28) · [来源](Lecture_Source_Map.md)
...
所以1 cm内约22.1%，4 cm内约63.2%，都不是简单的\(0.25L\)。完整推导见[N15](#N15)。若有散射竞争，而问题问“第一次碰撞就是吸收并发生在L以内”，可进一步求得
...
[T31–T32](Transcript_Corrected.md#T31) · [来源](Lecture_Source_Map.md)
...
“氢要乘2”是本例关键。核密度的单位与题设保持一致，采用每立方厘米。

### 课堂例2：纯铀-235金属的宏观总截面

题设密度19 g/cm³，摩尔质量近似235 g/mol。该例的微观截面有700 b与780 b两种记载，分别计算可看出它们对结果的影响：
...
课件第75–79页的铝、钠、NaCl、铀同位素混合物和UO₂算例，分别练习纯材料核密度、化学计量数、质量份额与同位素分配。特别是UO₂，应先辨别给出的富集度是否为“铀中235的质量份额”，不能将它当成整个氧化物中的份额。
...
[T33–T35](Transcript_Corrected.md#T33) · [来源](Lecture_Source_Map.md)
...
[T36–T38](Transcript_Corrected.md#T36) · [来源](Lecture_Source_Map.md)
...
本讲没有确认教材具体习题号。可从[教材索引](../../Global/Textbook_Index.md)定位G1 PDF45第5题（两种铀同位素丰度随时间变化），可用来练习两种核素衰变速度不同如何影响比例。公式条件可定位G1 PDF48式(2-3)、PDF52式(2-10)–(2-12)。

学完平均自由程后，可用课件第86页的英文例题检查理解。下一阶段将学习中子通量密度、反应率与平均截面。
